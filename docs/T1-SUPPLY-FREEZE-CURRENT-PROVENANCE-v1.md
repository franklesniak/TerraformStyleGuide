...
The canonical common-behavior source is [PSStyleGuide commit 97f126c](https://github.com/franklesniak/PSStyleGuide/tree/97f126c172221bb383a3d340d26338006ed1609d), tree `9f2a3cf3987b58549b1c5b96f025b1b9a41a0fcd`, delivered by [PR 205](https://github.com/franklesniak/PSStyleGuide/pull/205) and its [permanent handoff](https://github.com/franklesniak/PSStyleGuide/pull/205#issuecomment-5773736175). The focused Terraform baseline is commit `855580f008c8a27bc42f2cce426fc0d7459be9f5`, tree `e94eb84a206117df991543eec0d685c2c6a189e7`. [Issue 54](https://github.com/franklesniak/TerraformStyleGuide/issues/54) records the renewed direction decision and complete transfer requirements. Earlier direction changes remain recorded in [the prior reciprocal comparison](https://github.com/franklesniak/TerraformStyleGuide/issues/31#issuecomment-5769975041) and [the four counterpart obligations](https://github.com/franklesniak/TerraformStyleGuide/issues/31#issuecomment-5772954296).

| Role | Immutable input |
| --- | --- |
| Common recorder | PS `.github/workflows/Get-SupplyFreezeDigest.mjs`, blob `58ce7b0470a91589e6a6d99f26b30dc43cf21ce9`, SHA-256 `16449492cf091b04c35b90cd990a622ef440d01fec77c53b2799b9bc46f82b3b` |
| Common focused tests | PS `.github/workflows/Get-SupplyFreezeDigest.test.mjs`, blob `7b8c6c4d131a25387068fdb2bd9a26233773c23d`, SHA-256 `5b506f0d233ef457caff990721b83e4909a8dc25739079f1a463ceb034b9f7e4` |
| Common method | PS `docs/P1-SUPPLY-FREEZE-v1.md`, blob `cf463d264fd6b71bafb24bc8d98c0982bf616df8`, SHA-256 `9bc6b36d05116dc09adc49242a649729a7a690553913d31e7c93d79f0850d180` |
...
The current adaptation retains the source's common safety code, canonical JSON, parsers, installed-byte fold, advisory normalization, child outcomes, external housekeeping authority, confidentiality projections, root guards and owning-domain exception translators. It transfers canonical zero mount IDs, one reusable iterative parent/trie index per snapshot, and the complete owning-category topology catch. The paired acquisition recipe now gates all verification on effective-URL validation and native fetch success. The independent offline recipe retains the same complete Terraform verifier. Repository-specific substitutions bind the four current manifest identities, the exact current profile tuple, diagnostic labels, method path and output provenance. The focused tests use the same production functions and callers with corresponding TF data assertions. The separate Git verifier retains all three commits, five blobs and 13 Git calls, including both package pairs and the original historical recorder; one success marker follows all required checks. Candidate validation, review, landing and the later reciprocal comparison remain required before issue closure or a fixed-point claim.
...
| `currentObservation.npmProcesses[]`: `operation`, `nativeExit`, `signal`, `stderrLength` | string, integer/null, string/null, integer | Actual child outcomes. In a successful complete observation, `nativeExit` is an integer, `signal` is null, and only audit may have native status 1. An incomplete diagnostic observation can retain a failed npm-ls outcome, including null `nativeExit` and the terminating signal name. Every semantic stdout byte stream is exact-decoded before parsing or comparison. Stderr remains diagnostic-only: its length counts decoded child characters and public diagnostics expose fixed categories and lengths, not child text or raw bytes. |
...
Use a trusted private checkout and verified Git/Node tools. Exclude concurrent changes by the same user to the checkout, Node distribution, cache directory, their parents, or the visible mount namespace. Before starting any Node process, clear `NODE_OPTIONS`, `NODE_COMPILE_CACHE`, `NODE_V8_COVERAGE`, `NODE_REDIRECT_WARNINGS`, `NODE_DEBUG`, and `NODE_DEBUG_NATIVE`, and set `NODE_DISABLE_COMPILE_CACHE=1`. Node can configure output before the first JavaScript statement and write it at exit; a JavaScript refusal cannot undo those effects. This caller protocol applies independently to the recorder, test runner, and historical verifier. Bare invocation under arbitrary startup settings is not a read-only claim. The script cannot defend against code injected before its first statement, a hostile runtime, filesystem snapshots that are not atomic, permissions available to privileged actors or the same UID, concurrent remounts, ACL models not represented by POSIX mode bits, opaque provider-level storage aliases, or a hostile parent of the checkout. It creates no claim of physical fault injection or Windows PowerShell execution.
...
The recorder's external cache directory must be absolute, canonical, empty, owned by the recording UID, and mode `0700`. Symlink cache aliases and repository/toolchain overlap refuse. Before npm starts, the recorder reads at most 4 MiB of `/proc/self/mountinfo` data with at most 16,384 entries and maps the cache, both repository identities, the Node distribution, and protected descendant mounts to filesystem device/root coordinates. The parser accepts canonical nonnegative mount and parent IDs because supported Linux kernels can allocate mount ID `0` and can report parent ID `0`; unsafe integers, duplicate IDs, and parent cycles still refuse. See the [mount namespace format](https://man7.org/linux/man-pages/man7/mount_namespaces.7.html), the [Linux v6.6 mount-ID allocation](https://github.com/torvalds/linux/blob/v6.6/fs/namespace.c#L119-L126), and the [Linux v6.6 ID allocator](https://github.com/torvalds/linux/blob/v6.6/include/linux/idr.h#L237-L252). A bind-mount alias, malformed or oversized mount table, relevant stacked mount, or hidden/ambiguous relevant mapping refuses. This guard covers the stable mount namespace visible to the process; the caller exclusions above remain for concurrent remounts and opaque storage-provider aliases. Both the measured checkout and the physical recorder source repository are excluded, including diagnostic invocation through a script alias. Ancestors must be owned by root or the recording UID; writable ancestors require the sticky bit. The caller must prevent same-UID interference during the run. The complete initial installed-tree fold and its root, special-entry, and link checks run before the first npm subprocess; both installed-tree folds refuse special entries unconditionally. npm cache and logs are forced beneath that directory before every npm subprocess, including version and configuration probes; timing and update notification are disabled. Ambient repository-local cache/log paths cannot override these flags. Each npm child also removes all six Node startup variables listed above and forces `NODE_DISABLE_COMPILE_CACHE=1`; warnings remain visible as fixed categories and decoded-character lengths, while arbitrary child stderr text is withheld from public recorder diagnostics. Each audit receives a fresh environment that removes every case-insensitive registry alias and binds the already-checked value as canonical `NPM_CONFIG_REGISTRY`; npm's environment precedence keeps that value above project, user, and global configuration without publishing it in the process argument vector. Wrapper failures name only a fixed allowlisted operation and withhold all arguments. Strict `npm ls` problem diagnostics expose only safe native status plus fixed `missing`, `invalid`, `extraneous`, and `other` counts; package identifiers, versions, paths, unknown kinds, and other problem payload text are withheld. Observable active compile-cache, coverage, warning-redirection, or Node debug settings refuse with exit 2 before npm, but this check cannot undo runtime effects before module entry. Strict mode also refuses when `NODE_OPTIONS` is present, including an empty value; `--any-toolchain` bypasses that observable hygiene check and marks the output as not a freeze record. The caller protocol above remains mandatory in every mode because pre-entry code can erase the variable before this check. Empty values of the five output/debug variables are inactive; compile caching is inactive when its disable flag is `1`. No existing directory permissions are changed. npm's private log files can still contain sensitive data; retain and inspect that caller-owned external directory accordingly.

## Verify historical Git provenance separately

Choose one of the two procedures below. **Acquire and verify** explicitly prepares or repairs the historical inputs and then verifies them. **Verify offline** reuses complete local inputs without network access. A present commit does not prove that its blobs are present in a partial clone. Neither procedure changes the historical assertions.

### Acquire and verify

From the expected Terraform clone, run the complete block below with the verified `strNode` from above. Acquisition writes the Git object database before the read-only verification interval. The first Git command resolves the effective URL without contacting the remote and refuses when configuration would rewrite the canonical literal; see [`git ls-remote --get-url`](https://git-scm.com/docs/git-ls-remote#Documentation/git-ls-remote.txt---get-url). Keep repository, included, global, and system Git configuration unchanged from that check through the fetch. Concurrent same-UID configuration changes remain a caller exclusion. This check does not disable authentication, proxy, helper, redirect, or other transport behavior. Acquisition then refetches the three exact historical commits from the canonical explicit URL with `--no-tags --refetch` and no filter, including when the commit already exists. Do not replace the URL with a configured filtered remote. Each `&&` stops the block after a failed or mismatched URL check or a failed acquisition, before the verifier or its success marker can run. A failed fetch returns its native nonzero status, even when older local objects are complete. After successful acquisition, the verifier's native status is the block's status. There is no automatic local object probe or conditional-fetch interface.

```bash
strCanonicalRemote='https://github.com/franklesniak/TerraformStyleGuide.git'
strEffectiveRemote=$(git ls-remote --get-url "$strCanonicalRemote") && \
test "$strEffectiveRemote" = "$strCanonicalRemote" && \
git fetch --no-tags --refetch "$strCanonicalRemote" \
...
const historicalRecorderCommit = 'aae05282b57f093cec8b63e59138db72c982f10e';
const historicalRecorderPath = '.github/workflows/Get-SupplyFreezeDigest.mjs';
const historicalRecorderBlob = '05778c0eda0273a9217f7dc953795c2240473a14';
assert.equal(git('cat-file', '-t', historicalRecorderCommit).toString().trim(), 'commit');
assert.equal(
  git('rev-parse', `${historicalRecorderCommit}:${historicalRecorderPath}`).toString().trim(),
  historicalRecorderBlob,
);
const historicalRecorderBytes = git('cat-file', 'blob', historicalRecorderBlob);
assert.equal(historicalRecorderBytes.length, 345232);
assert.equal(
  createHash('sha256').update(historicalRecorderBytes).digest('hex'),
  '30384451d56d0c77f6bdb7c7bae4792a2434388e67de70c5501774b0feeecd25',
);
console.log('Current-profile inputs, historical T1 packages, and original recorder verification completed.');
NODE
```

### Verify offline

If all required objects are already local, use this independent block directly; no acquisition is required on each reverification. Missing objects refuse without lazy downloads. The verifier payload is identical to the acquisition recipe's payload; the focused test rejects drift between them.

The following independent procedure uses the verified `strNode` from above and a trusted Git executable on PATH. Every Git child receives a closed environment containing only that trusted PATH, the `C` locale, the fixed no-lazy-fetch/no-replacement/no-optional-lock settings, disabled system/global configuration, and disabled Trace2 targets. This prevents inherited object-routing or trace destinations from writing outside the measured repository during verification. Missing objects, a failed native command, an unexpected commit/path blob, or any length/hash mismatch terminate without a successful verification message. Only after both current-profile baseline objects, both historical T1 package objects, and the original historical recorder pass does it print one fixed all-input verification marker. It hashes raw stdout buffers, never a shell text pipeline or checkout conversion. It does not install, fetch, or update an index.

The pinned [Node 24.18.1 environment-variable reference](https://nodejs.org/download/release/v24.18.1/docs/api/cli.html#environment-variables) and [module compile-cache reference](https://nodejs.org/download/release/v24.18.1/docs/api/module.html#module-compile-cache) describe these startup output controls. The [Linux kernel mountinfo reference](https://www.kernel.org/doc/html/latest/filesystems/proc.html#proc-pid-mountinfo-information-about-mounts) defines the device, filesystem-root, mount-point, and parent fields; the [`proc_pid_mountinfo(5)` reference](https://man7.org/linux/man-pages/man5/proc_pid_mountinfo.5.html) describes stacked mounts and hidden paths. The [Git environment-variable reference](https://git-scm.com/docs/git#_environment_variables) and [Trace2 target reference](https://git-scm.com/docs/api-trace2) define the Git controls. The [npm configuration reference](https://docs.npmjs.com/cli/v11/using-npm/config/) defines the separate cache, logs-dir, and timing settings. The pinned [npm 11.16.0 audit exit-code implementation](https://github.com/npm/cli/blob/v11.16.0/node_modules/npm-audit-report/lib/exit-code.js) returns 0 or 1; valid-looking stdout does not authorize another native outcome.
...
| 5 | Malformed/inconsistent, invalid-UTF-8, or over-limit audit response, or a launched audit process with native status outside 0/1 or a signal. Subprocess launch failure remains exit 2. |
...
| 16 | Missing, repeated, unsafe, nonprivate, populated, overlapping, mount-aliased, or mount-topology-unverifiable external cache directory. Never bypassed. |
...
The reduced reciprocal matrix maps each of the 16 catalog rows exactly once. The four prior common gaps are implemented here; their destination evidence must bind final candidate bytes in the permanent handoff. This implementation table does not declare final reciprocal convergence. Unchanged generator/workflow foundation closure is not reopened.

| Row | Status | Normative/implementation locator, observed behavior, and applicability proof |
| --- | --- | --- |
| GF-PARAMETERS | Same | Unchanged argument contract, supported flags, checked registry environment and complete token withholding. ARGUMENT-PRIVACY and NPM-REGISTRY-TRANSPORT. |
| GF-DESTINATION | Same implementation; destination validation required | Fixed logical/physical roots and private external cache authority; canonical zero IDs and reusable bounded parent/trie indexes in validateCacheDirectory. CACHE-MOUNT-TOPOLOGY, CACHE-MOUNTINFO-READ and CACHE-MOUNT-GUARD-CALL cover alias/descendant/stack/hidden topology and causal scaling. |
| GF-CONTENT | Same common behavior | TF schema, profile and historical assertions remain intentional data differences. Common bounded projections, complete/incomplete semantics and nullable native outcomes match. Field/type map and AUDIT-PUBLIC-SUMMARY. |
| GF-SERIALIZATION | Same | canonicalize, hashField, installed/advisory framing and UTF-8/LF output retain common bytes. Different input graphs need not yield equal digests. |
| GF-WRITE | Inapplicable | Manual stdout recorder; no generator writer, candidate publication, replace, move or rollback implementation. External npm housekeeping is NODE-LOCK. |
| GF-FAILURE | Same implementation; destination validation required | The complete topology read/parse/index/coordinate/alias decision owns refusal16 before npm in strict and diagnostic modes. Guard-call mutations exercise category placement. Existing launch2/audit5, descriptor and privacy behavior remain. |
| GF-HOSTS | Same | Authenticated Linux/x64 Node24.18.1/npm11.16.0/umask0022. HOST proves Windows refusal only; no Windows/BSD supply measurement or skipped-case pass. |
| GF-VERSION | Inapplicable | No PowerShell script/version-marker change. Exact JavaScript bytes identify this recorder. |
| GF-NODE-LOCK | Same | TF manifest identities and profile tuple remain intentional data differences. npm authentication, child isolation, external housekeeping and installed-tree admission remain common. Contracts and locks remain unchanged. |
| GF-YAML | Inapplicable | No YAML package/API/parser or payload change; manifests and contract remain immutable validation inputs. |
| GF-ACTION-PINS | Inapplicable | No action pin, role, manifest or action runtime change. |
| GF-ACTION-INPUTS | Inapplicable | No action input/default change; the recorder is not inserted into a workflow. |
| GF-GIT | Same common gate; destination validation required | Acquire and verify checks the effective canonical URL and native refetch status before the full verifier. Verify offline uses identical payload bytes and no fetch. TF retains three commits/five blobs/13 verifier calls versus PS one commit/two blobs. No Git enters the recorder. |
| GF-GRAPH | Inapplicable | No trigger, permissions, needs/output graph, workflow writer or publisher change. |
| GF-CREDENTIALS | Same | Closed checked registry setting, operation-only npm wrappers and fixed public projections. No new token or credential authority. Git transport configuration stability, same-UID interference and private external logs retain explicit limits. |
| GF-EVIDENCE | Destination validation required | The focused suite transfers all coupled causal assertions and adds TF missing-object-group cases; paired recipes take two acquisition calls plus13 verifier calls. Retain native failures at calls9/13, wrong path at12, full final-byte gates and exact-tree before/after proof. Source passes do not establish destination results. |

Repository-specific differences are limited to the pinned input identities, TF schema/provenance/diagnostic labels, repository and method paths, retained historical record, and the additional historical package pair and original recorder. Equal failure strength follows from unchanged common checks and production-bound TF controls; no common safeguard is omitted. The repository maintainer owns these differences. Reopen on changed source, profile, manifest/lock, toolchain/platform assumption, advisory decision, test evidence, or a proposed difference that cannot prove equal security and failure strength. A new material exception requires its own evaluated decision. No dependency upgrade, shared runtime, workflow integration, advisory acceptance, protected instruction edit or issue22/24 change is included.

## Prior transfer and comparison snapshots (historical)

The following excerpts preserve what the earlier repair recorded on 2026-09-21. Their source is the [Terraform companion at commit 855580f](https://github.com/franklesniak/TerraformStyleGuide/blob/855580f008c8a27bc42f2cce426fc0d7459be9f5/docs/T1-SUPPLY-FREEZE-CURRENT-PROVENANCE-v1.md). These historical statements and their matrix do not classify the current candidate, reopen completed work, or override the current instructions and evidence requirements above. The excerpts retain their original wording and pins.

### Earlier source and repair snapshot
...
The current adaptation retains the source's common safety code, canonical JSON, parsers, installed-byte fold, advisory normalization, child outcomes, external housekeeping authority, confidentiality projections, root guards and owning-domain exception translators. F139-2 and F139-3 correct two common method descriptions beyond the pinned source: audit launch failure remains exit 2, and diagnostic child outcomes can carry null exit/string signal values. F139-4 adds one common cache-boundary implementation correction: Linux mount coordinates now prevent a bind-mounted external spelling from aliasing the repository or Node distribution. The Terraform repair is stronger now; the reverse PS sync-back and reciprocal comparison remain open under [PS issue 159](https://github.com/franklesniak/PSStyleGuide/issues/159) and the accepted sequencing in [TF issue 52](https://github.com/franklesniak/TerraformStyleGuide/issues/52). These are tracked common-convergence blockers, not Terraform-specific exceptions or unfinished Terraform repairs. Repository-specific substitutions bind the four current manifest identities, the exact current profile tuple, diagnostic labels, method path and output provenance. The focused tests use the same production functions and callers with corresponding TF data assertions. The separate Git verifier checks the current-profile baseline and the retained historical T1 package pair with the same closed child environment and byte/native-status rules; it prints success only after all required objects pass.
```

### Earlier reciprocal comparison snapshot
...
| Row | Status | Normative/implementation locator, observed behavior, and applicability proof |
| --- | --- | --- |
| GF-PARAMETERS | Same | Unchanged argument contract, supported flags, complete token withholding and checked registry environment. The new mount guard is classified under GF-DESTINATION. ARGUMENT-PRIVACY and NPM-REGISTRY-TRANSPORT. |
| GF-DESTINATION | Blocker | Fixed recorder root, `validateCacheDirectory`, physical/measured root exclusions and external private housekeeping authority remain common. F139-4 adds bounded visible-mount-coordinate admission that the pinned source lacks. Terraform now enforces the stronger common behavior; [PS issue 159](https://github.com/franklesniak/PSStyleGuide/issues/159) and [TF issue 52](https://github.com/franklesniak/TerraformStyleGuide/issues/52) retain the reverse sync-back and reciprocal comparison. INPUT-DIRECTORY-CONTROL, CACHE-MOUNT-TOPOLOGY, CACHE-MOUNTINFO-READ, and Linux success. |
...
| GF-VERSION | Inapplicable | No PowerShell script/version-marker change. Exact JavaScript bytes identify this recorder. |
| GF-NODE-LOCK | Blocker | TF manifest SHA/blob constants and current profile tuple remain intentional data differences; npm authentication, child isolation, and installed-tree admission remain common. F139-4 strengthens shared cache admission ahead of the pinned source. [PS issue 159](https://github.com/franklesniak/PSStyleGuide/issues/159) and [TF issue 52](https://github.com/franklesniak/TerraformStyleGuide/issues/52) retain the reverse sync-back and reciprocal comparison; this common convergence gap is not a repository-specific exception. |
