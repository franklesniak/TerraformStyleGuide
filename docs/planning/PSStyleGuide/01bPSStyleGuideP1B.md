...
checks exact dependency results and four unique cell attestations. The only
...
write-capable for the complete minimal writer job, but workflow code does not
materialize it into Git/process state until the exact push.
...
Record actual blocked-by relationships and exact merge commits. Before editing
workflows:
...
- validate exact ordinary-file identities and script versions for generator,
  helper, context manager, and harness; and
...
Do not change generator/helper/context/harness code, package/lock/hook/lint
configuration, source guides, generated artifacts, `.gitattributes`, or
Dependabot.

## Global workflow invariants

- `pull_request` runs for every pull request targeting `main`.
- `push` runs for every push to `main`.
...
retain provenance/action metadata evidence:
...
| Workflow | Job/step IDs | Action/release | Condition | Complete inputs |
| --- | --- | --- | --- | --- |
...
Both diagnostic steps use `continue-on-error: true`. Every action input not
listed in the table/normative expansions is prohibited; no unrecorded default
may become policy.

Use the complete full SHAs and adjacent release comments in YAML. Structurally
parse all workflow YAML and require exact role/input equality. Reject missing,
duplicate, extra, dynamic, mutable, wrong-repository, arbitrary-SHA,
wrong-comment/condition, swapped role, and unknown/weakened input.
...
12. emits only immutable artifact ID, bare upload digest, unique name,
    canonical `has_changes`, event SHA, and full target ref.
...
full ref, canonical `has_changes`, and completion marker.

Do not reuse one output name. Matrix completion order is not guaranteed.

### 5. Gate promotion with one terminal approval

One read-only approval job:

- depends on preparation, Markdown/Ubuntu validation, and Windows matrix;
- uses `if: always()`;
...
- compares every record to preparation;
...
Every checkout still uses `persist-credentials: false`.
...
`finally`. No bare push, `--force`, implicit destination, retry, or weaker
lease. Capture exit immediately. On success query exact remote ref and require
new commit; on rejection fail unchanged.

### 10. Add controlled negative drills

Before enabling `main`, use unique temporary branch/artifact fixtures:
...
- only two workflow files changed/staged;
...
- package/lock/hook/lint config/generated artifacts unchanged;
- YAML parses; and
...
- [ ] Actions/conditions/inputs equal one final exact role table.
...
- [ ] Token exists job-wide but is never persisted/materialized outside push.
...
- [ ] Only `build.yml` and `markdownlint.yml` changed/staged.
- [ ] P1B↔T1B has no unexplained blocker.
- [ ] P2 records P1B's exact merge commit.
