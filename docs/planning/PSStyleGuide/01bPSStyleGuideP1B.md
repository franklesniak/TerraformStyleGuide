...
never-merged evidence ref. On that ref use the real `build.yml` with an exact
literal-only patch changing the authorized target/predicate from
`refs/heads/main` to the recorded evidence ref, plus one safe test-owned source
delta that produces a changed candidate. A structural semantic comparator must
prove production and evidence workflow trees are identical except that one
allowlisted literal and evidence metadata; action roles, scripts, permissions,
needs, inputs, and writer algorithm remain byte/semantic equal.
...
then delete the remote evidence ref. The evidence commit is never merged and
no evidence workflow file exists. Final validation proves production
`build.yml` has the exact `main` literal, the evidence ref is absent, and the
semantic comparator/evidence bundle identify the reviewed production commit.
If landed production differs, rerun the evidence on a new isolated ref.
...
predicate, candidate/diagnostic artifact contracts, credential invariants, and
absence of remote/dynamic workflows, cache, auto-extraction, persisted
credentials, transitive output use, or unknown roles.
...
real-writer evidence, diagnostics, and coverage/cost.
...
credential cleanup, remote deletion, and absence of evidence state. Final
production validation uses the exact `main` topology and retains run URLs/IDs.
...
- [ ] Real `build.yml` isolated-ref evidence proves success/negative cases,
      exact one commit, ref deletion, and no copied workflow.
...
deletion proof, final scope proof, and P1B↔T1B matrix. P2 compares these landed
interfaces with its assumptions before changing content.
