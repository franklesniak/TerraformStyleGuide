...
| T1A-Z-03 | `archive.trailer.decoy-rejected` | decoy end-of-central-directory record at `EOF-22` declaring a comment length no bytes satisfy, with a conforming trailer behind it and a fat directory the decoy points at | `archive/trailer-decoy`; candidate absent; refused before `ZipArchive` construction |
| T1A-Z-04 | `archive.trailer.zip64-gate-rejected` | single trailer with both disk fields at `0xFFFF` and a Zip64 locator hidden in the final central-directory record's comment | `manifest/zip64-locator`; candidate absent; refused before `ZipArchive` construction |
...
| T1A-W-06 | `download.entry.unclassifiable-rejected` | unclassifiable download entry; on Linux a `mkfifo` named pipe | `download/entry-unclassifiable` reached **before any open of the entry**; candidate absent; the run completes rather than blocking |
