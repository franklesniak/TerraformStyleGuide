...
| Freeze script SHA-256 | `30384451d56d0c77f6bdb7c7bae4792a2434388e67de70c5501774b0feeecd25` | script `script.sha256` |
...
| `2` | npm installation entry the recorder does not solely control | **no** | a regular file inside the npm installation — or the installation root directory, or the launcher beside `node` — with more than one hard link, so a second path can rewrite the bytes, or owned by a different uid than the recording process, whose owner can substitute an npm support module between the subprocess calls and restore the reviewed bytes before the final fold. This mirrors exit `14` for the installed tree, at the exit code the installation's own refusals already use; it is a self-consistency check, not a comparison against `REVIEWED_NPM_TREE_SHA256`, so `--any-toolchain` does not waive it. The hard-link test is applied to **regular files only** — a directory's link count counts its subdirectories, and a symlink is not a byte source — while the uid test covers the root, the launcher and every folded file. `uid` itself is not hashed, for the reason exit `14` gives: it is a property of the machine, so a mismatch is refused rather than folded. Measured: a walk file given a foreign uid exits `2` naming `owned by uid 1000, but this recorder runs as uid 0`; a walk file given a second hard link exits `2`; the launcher and the root each exit `2` on a foreign uid; and the root's own hard-link count greater than one does **not** fire, because directories are exempt |
...
| `15` | Recorded manifest is not a regular file, or a manifest or `.npmrc` another path can rewrite | **no** | `package.json` or `package-lock.json` present as a symlink, directory, FIFO, socket or device node — the same closed set exit `13` applies to `.npmrc`. The FIFO is the case that makes this a correctness rule rather than a tidiness one: its bytes are a stream supplied per open, so the snapshot reads the reviewed bytes and `npm ls`/`npm audit` then consume whatever is written next, while the quiescence sweep sees only an unchanged inode and change time. Both are opened *by name* by `npm ls` and `npm audit`, and a link's target sits in a directory outside the swept set, which can be renamed aside, replaced, and restored before the final sweep — leaving the link and its original target comparing equal while npm answered from other bytes. The same rule already applies to a project `.npmrc` (exit `13`); this closes the gap where the more load-bearing inputs, the ones whose hashes are compared, were held to the weaker rule. Beyond shape, a manifest or the project `.npmrc` that **is** a regular file but that **another path can rewrite** — one with more than one hard link, or owned by a different uid than the recording process — is refused here as well, checked on the **open descriptor** whose inode npm reads (`fstat`, not a by-name `lstat` a swap can outrun), because an alternate write path lets npm read other bytes between the snapshot and the final read while the recorded change time still compares equal. This is the descriptor-bound counterpart of the tree's exit `14` rule; for a manifest it also closes a pre-existing gap in which a **regular-file** manifest another path could rewrite was folded without the check, and for `.npmrc` — whose non-regular shapes exit `13` — the rewrite case lands here at exit `15`. Measured: a symlinked `package.json` exits `15`, and restoring a regular file returns exit `0`; a manifest given a second hard link exits `15`; and a `.npmrc` with a second hard link, and one owned by a foreign uid, each exit `15` |
